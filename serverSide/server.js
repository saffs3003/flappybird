...
app.get("/api/all-songs", (req, res) => {
  db.all("SELECT songs.*, users.name AS artistName FROM songs JOIN users ON songs.artistId = users.id ", (err, rows) => {
...
app.get("/api/approved-songs", (req, res) => {
  db.all("SELECT songs.*, users.name AS artistName FROM songs JOIN users ON songs.artistId = users.id  WHERE isApproved = 1", (err, rows) => {
    if (err) return res.status(500).json({ message: "Error fetching songs" });
    res.json(rows);
  });
});
...
app.post('/api/approve-event/:id', (req, res) => {
  ...
  db.run(query, [id], function (err) {
    if (err) {
      return res.status(500).json({ message: "Error approving events", error: err.message });
    }
...
app.get('/api/artist/event/:id', (req, res) => {
  ...
  db.all(query, [id], (err, rows) => {
    if (err) {
      return res.status(500).json({ message: "Error fetching Events of artist", error: err.message });
    }
...
app.get("/api/unapproved-events", (req, res) => {
  db.all("SELECT events.*, users.email AS artistEmail FROM events JOIN users ON events.artistId = users.id WHERE events.isApproved = 0;", (err, rows) => {
